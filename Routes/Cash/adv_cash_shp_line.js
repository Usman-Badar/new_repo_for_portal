...
router.post('/cash/shipping/create', ( req, res ) => {
    ...
    const { line, pr_id, previous_slip, emp_id, request_to, company_code, location_code, reason, amountInWords, amount, d_o, lolo, detention, damage_dirty, other, other_specification } = req.body;
    const d = new Date();
                        ...
                        connection.query(
                            ...
                            ( err, rslt ) => {
                                ...
                                }else 
                                {
                                    ...
                                    {
                                        connection.query(
                                            "INSERT INTO `db_cash_receipts`(`shp_line_adv`, `line`, `d_o`, `lolo`, `detention`, `damage_dirty`, `other_purpose_amount`, `other_purpose_specification`, `previous_slip`, `pr_id`, `serial_no`, `series_year`, `emp_id`, `submit_date`, `submit_time`, `amount`, `amount_in_words`, `company`, `location`, `reason`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);",
                                            [ 'Y', line, d_o, lolo, detention, damage_dirty, other, other_specification, previous_slip == 'null' || previous_slip == 'undefined' ? null : previous_slip, pr_id == 'null' || pr_id == 'undefined' ? null : pr_id, serial_no, financial_year, emp_id, d, d.toTimeString(), amount, amountInWords, company_code, location_code, reason ],
                                            ( err, result ) => {
                                                if( err )
                                                {
                                                    connection.rollback(() => {console.log(err);connection.release();});
                                    ...
                                    {
                                        connection.query(
                                            "INSERT INTO `db_cash_receipts`(`shp_line_adv`, `line`, `d_o`, `lolo`, `detention`, `damage_dirty`, `other_purpose_amount`, `other_purpose_specification`, `serial_no`, `series_year`, `emp_id`, `submit_date`, `submit_time`, `amount`, `amount_in_words`, `company`, `location`, `reason`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);" +
                                            "INSERT INTO `db_cash_balance`(`emp_id`, `balance`, `last_updated`) VALUES (?,?,?);",
                                            [ 'Y', line, d_o, lolo, detention, damage_dirty, other, other_specification, serial_no, financial_year, emp_id, d, d.toTimeString(), amount, amountInWords, company_code, location_code, reason, emp_id, balance, d ],
                                            ( err, result ) => {
                                                if( err )
                                                {
                                                    connection.rollback(() => {console.log(err);connection.release();});
