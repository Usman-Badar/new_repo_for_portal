...
router.post('/cash/advance/create', ( req, res ) => {
    ...
    //             }else
    //             {
    //                 const message = result[2][0].name + " has requested for advance cash for PKR (" + amount.toLocaleString('en') + ")";
    //                 administrativeNotifications( link, owner, message );
    //                 SendWhatsappNotification( null, null, "Hi " + result[0][0].name, "You have requested for advance cash, your request has been forwarded to " + result[1][0].name + ". Please wait... while " + result[1][0].name + " is reviewing your request.", result[0][0].cell );
    //                 SendWhatsappNotification( null, null, "Hi " + result[1][0].name, result[0][0].name + " has request for advance cash on portal for PKR " + amount.toLocaleString('en') + ", the request is being in awaiting for approval. Kindly review.", result[1][0].cell );
    //                 SendWhatsappNotification( null, null, "Hi " + result[2][0].name, result[0][0].name + " has requested for advance cash on your behalf for PKR " + amount.toLocaleString('en') + ". " + result[1][0].name + " reviewing your request, please wait.", result[2][0].cell );
    //             }
    //         }
    //     );
    // }
    ...
    //                                 {
    //                                     connection.query(
    //                                         "INSERT INTO `db_cash_receipts`(`emp_id`, `amount`, `amount_in_words`, `company`, `location`, `reason`, `recorded_by`, `recorded_date`, `recorded_time`, `approved_by`) VALUES (?,?,?,?,?,?,?,?,?,?);",
    //                                         [ employee, amount, amountInWords, company_code, location_code, reason, emp_id, d, d.toTimeString(), request_to ],
    //                                         ( err, result ) => {
    //                                             if( err )
    //                                             {
    //                                                 connection.rollback(() => {console.log(err);connection.release();});
    ...
    //                                         [ employee, amount, amountInWords, company_code, location_code, reason, emp_id, d, d.toTimeString(), request_to, employee, balance, d ],
    //                                         ( err, result ) => {
    //                                             if( err )
    //                                             {
    //                                                 connection.rollback(() => {console.log(err);connection.release();});
...
router.post('/cash/load/requests', ( req, res ) => {
    ...
    db.query(
        ...
        LEFT OUTER JOIN employees record ON db_cash_receipts.recorded_by = record.emp_id \
        ...
        " + ( accessKey === 1 ? "" : "WHERE approved_by = ? OR recorded_by = ? OR cashier = ? OR db_cash_receipts.emp_id = ?" ) + " ORDER BY `id` DESC;",
...
router.post('/cash/load/request/cancel', ( req, res ) => {
    ...
    db.query(
        "UPDATE db_cash_receipts SET approved_by = ?, status = ?, approved_date = ?, approved_time = ?, hod_remarks = ? WHERE id = ? AND recorded_by = ?;",
...
router.post('/cash/validation', ( req, res ) => {
    const { receiving_person, receiving_person_contact, request_id, amount, other, passcode, employee, emp_id, template1, template2 } = req.body;
    const d = new Date();
    let other_person = 0;
    if ( other === 'yes' )
    {
                                ...
                                ( err ) => {
                                    ...
                                    }else
                                    {
                                            ...
                                            fs.mkdir('assets/portal/assets/AC/' + request_id + '/',
                                                ...
                                                (err) => {
                                                    if (err) {
                                                        ...
                                                        arr[0].mv('assets/portal/assets/AC/' + request_id + '/front.png', (err) => {if (err){connection.rollback(() => {console.log(err);connection.release();});res.status(500).send(err);res.end();}})
                                                        arr[1].mv('assets/portal/assets/AC/' + request_id + '/back.png', (err) => {if (err){connection.rollback(() => {console.log(err);connection.release();});res.status(500).send(err);res.end();}})
                                                    }
                                                }
                                            )
                                        }
                                        connection.query(
                                            ...
                                            "UPDATE db_cash_receipts SET receival_date = ?, receival_time = ?, status = ?, emp_finger_print = ?, cashier_finger_print = ?, other = ?, received_person_name = ?, received_person_contact = ?, cnic_front = ?, cnic_back = ? WHERE id = ? AND cashier = ?;",
                                            [ parseFloat(amount), d, employee, d, d.toTimeString(), 'issued', fingerprint2Name, fingerprint1Name, other_person, receiving_person == 'null' ? null : receiving_person, receiving_person_contact == 'null' ? null : receiving_person_contact, 'front.png', 'back.png', request_id, emp_id ],
                                            ( err, rslt ) => {
                                                if( err )
                                                {
                                                    connection.rollback(() => {console.log(err);connection.release();});
...
router.post('/cash/request/clearance', ( req, res ) => {
    const { request_id, after_amount, recorded_by, employee, amount, emp_id } = req.body;
    const d = new Date();
    db.getConnection(
        ( err, connection ) => {
            connection.beginTransaction(
