...
function Update_Emp_Att_Status(emp_id, m_d, date)
{
    db.query(
        "SELECT time_in, grace_in_minutes FROM `employees` WHERE time_in IS NOT NULL AND emp_id = " + emp_id,
        ( err, rslt ) => {
            let limit = rslt.length;
            let count = [];
            ...
            function Mark()
            {
                ...
                if (m_d.toLowerCase() === 'm') {
                    ...
                    db.query(
                        ...
                        JOIN employees b ON a.emp_id = b.emp_id \
                        SET a.status = 'Present' \
                        WHERE b.emp_id = ? AND b.time_in = ? AND a.time_in < ? AND a.emp_date = ? AND a.time_out IS NOT NULL AND a.status != 'OFF' AND a.status != 'leave' AND a.status != 'short leave' AND a.edit_by IS NULL; \
                        UPDATE emp_attendance \
                        SET emp_attendance.status = 'Present' \
                        WHERE emp_attendance.emp_id = ? AND emp_attendance.emp_date = ? AND emp_attendance.status = 'Absent' AND emp_attendance.time_in IS NOT NULL AND emp_attendance.status != 'OFF' AND emp_attendance.status != 'leave' AND emp_attendance.status != 'short leave' AND emp_attendance.edit_by IS NULL; \
                        UPDATE emp_attendance a \
                        JOIN employees b ON a.emp_id = b.emp_id \
                        SET a.status = 'Late' \
                        WHERE b.emp_id = ? AND b.time_in = ? AND a.time_in > ? AND a.emp_date = ? AND a.time_in IS NOT NULL AND a.time_out IS NOT NULL AND a.status != 'OFF' AND a.status != 'leave' AND a.status != 'short leave' AND a.edit_by IS NULL;", 
                        [ 
                            emp_id, rslt[count.length].time_in, time, date, 
                            emp_id, date, 
                            emp_id, rslt[count.length].time_in, time, date 
                        ],
                        ( err ) => {
                            ...
                            }else
                            {
                                if ( ( count.length + 1 ) === limit )
                                {
                                    db.query(
                                        "UPDATE emp_attendance \
                                        SET emp_attendance.status = 'Late' \
                                        WHERE emp_attendance.emp_id = ? AND emp_attendance.emp_date = ? AND emp_attendance.status = 'Present' AND emp_attendance.time_out IS NULL AND emp_attendance.emp_date != CURDATE() AND emp_attendance.status != 'OFF' AND emp_attendance.status != 'leave' AND emp_attendance.status != 'short leave' AND emp_attendance.edit_by IS NULL;",
                                        [ emp_id, date ],
                                        ( err ) => {
                                            if( err )
                                            {
...
function UpdateLateForDate(emp_id, date)
{
    db.query(
        "SELECT emp_id, time_in, grace_in_minutes FROM `employees` WHERE time_in IS NOT NULL AND emp_status = 'Active' AND emp_id = ? AND location_code != 4 AND company_code != 10;",
        [emp_id],
        ( err, rslt ) => {
            console.log(rslt);
            let limit = rslt.length;
            ...
            function Mark()
            {
                ...
                db.query(
                    ...
                    JOIN employees b ON a.emp_id = b.emp_id \
                    SET a.status = 'Late' \
                    WHERE b.time_in = ? AND a.time_in >= ? AND a.emp_date = ? AND a.time_in IS NOT NULL AND a.time_out IS NOT NULL AND a.emp_date != CURDATE() AND a.emp_id = ? AND a.status = 'Present';", 
                    [ rslt[count.length].time_in, time, date, emp_id],
                    ( err ) => {
                        if( err )
                        {
                            console.log(err);
...
module.exports = {
    ...
    Update_Emp_Att_Status: (emp_id, m_d, date) => Update_Emp_Att_Status(emp_id, m_d, date)
};