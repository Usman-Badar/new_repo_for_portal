...
router.post('/get_all_notices', ( req, res ) => {
    db.query(
...
router.post('/notice/new', ( req, res ) => {
    ...
    db.query(
        "INSERT INTO `tbl_notices`(`title`, `url`, `type`) VALUES (?,?,?);",
        [title, `${d}_${file.name}`, type],
        (err) => {
            if( err ) {
                console.log(err);
                res.status(500).send(err);
                ...
                if( err ) {
                    ...
                    function sendToEmp() {
                        ...
                        if ((empCount+1) === empLimit) {
                            if ((count+1) === limit) {
                                db.query(
                                    "UPDATE `tbl_notices` SET whatsapp_sent = ? WHERE url LIKE '%" + url + "%'",
                                    [1],
                                    (err) => {
                                        if( err ) {
                                            console.log(err);
